...
function buildSucculent(preset: { succulent: Partial<(typeof SUCCULENT_PRESETS)[number]['succulent']> }, seed: number) {
...
describe('welded succulent is a single closed manifold', () => {
  for (const preset of SUCCULENT_PRESETS) {
    for (const seed of [1, 7, 42]) {
      it(`${preset.name} seed ${seed}`, () => {
        ...
        expect(report.eulerCharacteristic, 'Euler characteristic').toBe(2);
        expect(report.genus).toBe(0);
        // Desert plants are entirely quads: ladder caps triangulate to quads.
        expect(report.quadRatio).toBe(1);
        expect(stats.dropped, 'dropped organs').toBe(0);
        // Every organ but the base was welded through a window.
        expect(stats.junctions).toBe(stats.organs - 1);
        expect(stats.stems + stats.pads + stats.leaves).toBeGreaterThan(0);
        expect(height).toBeGreaterThan(0.03);
        expect(groundDepth).toBeGreaterThan(0);
        // Wind attributes: in range, sway weight reaches 1 at the top, no limb
        // weight or flutter on the base. (Counted in a plain loop: one expect
        // per vertex would dominate the test time on 400k-vertex plants.)
        const n = mesh.vertexCount;
        let maxHeight = 0;
        let outOfRange = 0;
        let baseMoving = 0;
        for (let i = 0; i < n; i++) {
          ...
          if (mesh.wind[i * 4] > maxHeight) maxHeight = mesh.wind[i * 4];
          if (mesh.levels[i] === 0 && (mesh.wind[i * 4 + 1] !== 0 || mesh.wind[i * 4 + 3] !== 0)) baseMoving++;
        }
        expect(outOfRange, 'wind attributes out of [0, 1]').toBe(0);
        expect(baseMoving, 'base vertices with limb/flutter weight').toBe(0);
        expect(maxHeight).toBeCloseTo(1, 5);
      });
    }
  }
  ...
  it('succulent presets carry the succulent kind and go through the shared pipeline', () => {
    const succulents = PRESETS.filter(isSucculent);
    expect(succulents.length).toBe(SUCCULENT_PRESETS.length);
    const p = cloneParams(succulents.find((g) => g.name === 'Golden Barrel')!);
    p.seed = 3;
    const r = generateTree(p);
    expect(r.skeleton).toBeNull();
    ...
    expect(r.summary.height).toBeGreaterThan(0.3);
    expect(r.buffers.index.length).toBe(r.mesh.quadCount * 6 + r.mesh.triCount * 3);
  });

  it('is deterministic for a given seed and differs across seeds', () => {
    const a = buildSucculent(SUCCULENT_PRESETS[0], 9);
    const b = buildSucculent(SUCCULENT_PRESETS[0], 9);
    const c = buildSucculent(SUCCULENT_PRESETS[0], 10);
    expect(a.mesh.positions.length).toBe(b.mesh.positions.length);
    expect(a.report.faces).toBe(b.report.faces);
    const sum = (arr: ArrayLike<number>) => {
      let t = 0;
  ...
  it('exports a succulent as quads to OBJ and GLB', () => {
    const { mesh } = buildSucculent(SUCCULENT_PRESETS.find((g) => g.name === "Bishop's Cap")!, 1);
    const leaves = new LeafMesh();
    const obj = toOBJ(mesh, leaves, 'succulent');
    const lines = obj.split('\n');
    expect(lines.filter((l) => l.startsWith('v ')).length).toBe(mesh.vertexCount);
    expect(lines.filter((l) => l.startsWith('f ') && l.trim().split(/\s+/).length === 5).length).toBe(mesh.quadCount);
    const glb = toGLB(mesh, leaves, 'succulent');
    const dv = new DataView(glb);
    expect(dv.getUint32(0, true)).toBe(0x46546c67);
    expect(dv.getUint32(8, true)).toBe(glb.byteLength);
  });
