...
// 31. kernel regression: primitive and profile solids expose a closed half-edge B-rep; sub-element edits stay parametric, transform-aware and serializable
{ if(M.anyTool&&M.anyTool())M.selectTool();M.newDoc({silent:true});
  M.runCmd('box 40 30 20');let b=M.doc.figures.find(f=>f.op==='box');let m=M.meshOf(b),br=m.brep;ok(br&&br.schema==='solidarc.brep.v2'&&br.closed&&br.valid,'primitive box has a validated B-rep');ok(br.halfEdges.length===36&&br.halfEdges.every(h=>h.twin!==null),'box half-edges all have twins');ok(br.faces.every(f=>f.halfEdges&&f.halfEdges.length),'faces retain half-edge adjacency');
  const pos0=[...b.pos],vi=M.topo(b).verts.findIndex(v=>v.p[0]<0&&v.p[1]<0&&v.p[2]===0);M.sub_.sel.clear();M.sub_.sel.set('v',{fig:b.id,kind:'vertex',idx:vi});const st=M.subBegin();ok(st&&st.start.length===1,'body vertex drag captures one persistent reference');M.subApply(st,[5,-2,3]);M.subEnd();ok(b.vertexOps&&b.vertexOps.length===1&&b.pos.every((v,i)=>v===pos0[i]),'vertex drag stores vertexOps and leaves the body transform untouched');m=M.meshOf(b);ok(m.kernel.valid&&m.brep.faces.length===6&&m.brep.edges.length===12,'vertex operation replays across faces and edges without opening the solid');
  M.sub_.sel.clear();M.newDoc({silent:true});M.runCmd('box 40 30 20');const bp=M.doc.figures.find(f=>f.op==='box');const top=M.topo(bp).faces.findIndex(f=>f.key==='cap:top');M.faceOpApply([M.faceInfoWorld(bp,top)],'push',4);ok(!bp.faceOps&&bp.params.h===24,'primitive cap push remains a height edit');ok(M.meshOf(bp).kernel.valid,'primitive cap push remains watertight');
  M.newDoc({silent:true});M.runCmd('cylinder 10 30');const cy=M.doc.figures.find(f=>f.op==='cylinder');const side=M.topo(cy).faces.findIndex(f=>f.kind==='cylinder');const ch=M.faceInfoWorld(cy,side);M.faceOpApply([ch],'push',2);ok(cy.params.r>10&&(!cy.faceOps||!cy.faceOps.length)&&M.meshOf(cy).kernel.valid,'analytic cylinder side push edits the radius without polygonizing it');
  M.newDoc({silent:true});M.startOp('rect');M.toolClick(390,300);M.toolClick(510,380);const rN=M.doc.figures.filter(f=>f.kind==='curve').pop();M.selectTool();M.doc.sel=new Set([rN.id]);M.startOp('extrude');M.solidKey({key:'-'});M.solidKey({key:'1'});M.solidKey({key:'5'});M.solidKey({key:'Enter'});const exN=M.doc.figures.find(f=>f.op==='extrude');m=M.meshOf(exN);ok(exN.params.height===-15&&m.kernel.valid&&m.brep.closed,'negative extrusion keeps the profile at the sketch plane and builds a closed solid');ok(Math.abs(M.measure(exN).v-M.polyArea(M.curvePlane(rN).pts)*15)<1e-6,'negative extrusion volume is positive and dimensionally correct');
  exN.scl=[2,3,4];M.invalidate(exN.id);ok(Math.abs(M.measure(exN).v-M.polyArea(M.curvePlane(rN).pts)*15*24)<1e-6,'solid measure applies the transform determinant');const js=M.docJSON();const exId=exN.id;M.loadDocJSON(js);ok(M.byId(exId).params.height===-15&&M.meshOf(M.byId(exId)).kernel.valid,'B-rep feature state survives document save/load');M.sub_.sel.clear();M.doc.sel.clear(); }


// 32. cap bevel regression: a narrow concave profile must clamp an impossible offset,
// retain every cap region, and stay closed when both cap loops are treated together.
{ if(M.anyTool&&M.anyTool())M.selectTool();M.newDoc({silent:true});
  const P=[[-30,-30],[30,-30],[30,-10],[5,-10],[5,10],[30,10],[30,30],[-30,30],[-30,10],[-5,10],[-5,-10],[-30,-10]];
  M.startOp('rect');M.toolClick(390,300);M.toolClick(510,380);const R=M.doc.figures.filter(f=>f.kind==='curve').pop();R.params.pts=P;R.params.closed=true;R.params.bulge=[];M.invalidate(R.id);M.selectTool();M.doc.sel=new Set([R.id]);M.startOp('extrude');M.solidKey({key:'2'});M.solidKey({key:'0'});M.solidKey({key:'Enter'});const b=M.doc.figures.filter(f=>f.op==='extrude').pop();
  const check=(ed,label)=>{b.edits=ed;M.invalidate(b.id);M.topoCache.clear();const m=M.meshOf(b),v=m.kernel&&m.kernel.validation;ok(m.kernel.valid&&m.brep.closed&&v.manifold&&v.boundaryEdges===0&&v.degenerate===0,label+' stays watertight');const top=m.brep.faces.find(f=>f.key==='cap:top'),bot=m.brep.faces.find(f=>f.key==='cap:bot');ok(top&&bot&&top.tris.length>=P.length-2&&bot.tris.length>=P.length-2,label+' retains complete concave cap triangulation');return m;};
  check([{type:'fillet',key:'top:all',r:8},{type:'fillet',key:'bot:all',r:8}],'concave top+bottom fillet');
  check([{type:'chamfer',key:'top:all',r:1000},{type:'fillet',key:'bot:all',r:1000}],'oversized mixed chamfer+fillet');
  check([{type:'fillet',key:'top:0',r:8},{type:'chamfer',key:'bot:all',r:8},{type:'fillet',key:'side:0:0',r:8}],'mixed selected cap/profile edits');
  M.sub_.sel.clear();M.doc.sel.clear();
  // A pair of genuine sketch holes exercises the same bridge/triangulation path.
  M.newDoc({silent:true});M.startOp('rect');M.toolClick(350,250);M.toolClick(550,450);M.startOp('circle');M.toolClick(420,350);M.toolClick(440,350);M.startOp('circle');M.toolClick(480,350);M.toolClick(500,350);const hs=M.doc.figures.find(f=>f.kind==='sketch'),hc=hs.children.map(M.byId),hr=hc.find(c=>c.ctype==='poly'),hcirc=hc.filter(c=>c.ctype==='circle');hr.params.pts=[[-100,-80],[100,-80],[100,80],[-100,80]];hcirc.forEach((c,i)=>{c.params.cx=i?-45:45;c.params.cy=0;c.params.r=20;});M.invalidate(hs.id);M.selectTool();M.doc.sel=new Set([hs.id]);M.startOp('extrude');M.solidKey({key:'2'});M.solidKey({key:'0'});M.solidKey({key:'Enter'});const hb=M.doc.figures.filter(f=>f.op==='extrude').pop();hb.edits=[{type:'fillet',key:'top:all',r:8},{type:'fillet',key:'bot:all',r:8}];M.invalidate(hb.id);M.topoCache.clear();const hm=M.meshOf(hb);ok(hm.kernel.valid&&hm.brep.closed&&hm.kernel.validation.boundaryEdges===0,'concave-cap offset code also closes a multi-hole profile');ok(hm.brep.faces.find(f=>f.key==='cap:top').tris.length>100&&hm.brep.faces.find(f=>f.key==='cap:bot').tris.length>100,'multi-hole caps retain both hole boundaries');
  M.sub_.sel.clear();M.doc.sel.clear(); }
