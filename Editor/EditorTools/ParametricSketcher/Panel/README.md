...
- `solidBuild()` is the shared analytic **boundary-representation** path for boxes, cylinders and profile extrusions. A box is a four-line prism profile; a cylinder is a rational circular profile (two semicircular bulges); a sketch line sweeps to a plane, an arc / circle sweeps to a cylinder (cone under draft), and caps are planes. A cylinder is therefore 3 faces, 2 circular edges and no vertices; the side is shaded with per-vertex analytic normals (gradient-filled quads) so it reads as one smooth curved surface, with a silhouette line computed per frame instead of facet lines.
- Faces / edges / vertices come from the B-rep (`mesh.brep`), keyed stably (`side:2`, `cap:top`, `h1.top:0` …) so face picking selects the whole curved side and edits survive rebuilds. The kernel publishes `solidarc.brep.v2`, directed half-edges with `next` / `prev` / `twin`, face-edge-vertex adjacency, manifold/closed flags, boundary-edge count, degenerate-triangle count and signed volume. Tangent edges (fillet boundaries) draw faint; sharp edges draw dark.
...
- Measure reports faces / edges / tangent edges, feature counts and the tessellated B-rep volume. Transform scale is applied with the determinant, so inspector quantities remain physical after non-uniform scaling.
- Cap-edge fillets are **Blender-style**: only the picked edge is bevelled; a neighbouring corner-fillet arc is not rolled into — the band dies out across it with a triangular blend (offset ramps from the bevel radius to zero along the arc, never past the arc centre). Offset rings use a topology-aware line/arc construction, reject reversed, collapsed, touching, or self-intersecting loops, and binary-clamp the requested distance before tessellation; this prevents concave-profile spikes and preserves the original profile when no positive radius is possible. Tangent joints are handled analytically, so a fillet next to an existing vertical-edge fillet cannot spike or fly off. Tangent (fillet-boundary) edges are not valid fillet targets. With the Fillet / Chamfer tool active, a click prefers edges and vertices over faces, and clicking an element that is not part of the current selection operates on that element alone — a whole edge loop is only bevelled when you explicitly pick a face (or select the loop).
...
- **Matcap** is now the default shading: a studio-style capture for machined parts — top-left key light, cool rim, soft under-fill, a tight clear-coat highlight and cavity darkening where the normal turns away; selection is a mint-green tint. Plastic / Flat / Wire remain.
...
- **Vertex bevel / chamfer** (Blender-style): in Vertex mode select a body corner (or hover it inside the Fillet/Chamfer tool), press **B** for a spherical corner or **⇧B** for a flat cut, drag / type the distance. Stored as a `vbevel` feature on the body (Inspector → Face features, removable, undoable); watertight. Vertex drags use the separate B-rep vertex-edit list and preserve the original vertex key.
...
- **E on a planar face** of a solid (hover-click in the Extrude tool, or pre-select faces in Face mode and press E) pushes that face along its own normal: drag or type the distance; negative = pocket. A selected cylindrical side also performs an analytic radial push by editing the cylinder radius; it never becomes a polygon-only body. Pushing a whole cap simply extends the adjacent walls (no extra faces); pushing an inset region grows a boss / cuts a pocket with new wall faces (smooth cylindrical walls on round regions).
...
- **Moving a face, edge or vertex (gizmo G / arrows, or E on a face) edits the topology-aware feature directly** — it is *not* a whole-body translation: top cap → height; bottom cap → base offset (the opposite cap stays put); a box side → width/depth plus its centre; a cylinder side → radius; a side face of a profile extrusion → the profile edge moves in the sketch. Vertices are stored as persistent `vertexOps` keyed to the original kernel vertex and replayed over every rebuild; adjacency, trims and half-edges are remapped together. Only faces that were already produced by a feature (an inset region, a boss top) get a push feature.
...
- Still out of scope for this HTML kernel: general multi-body booleans and arbitrary-vector face push. Planar face push, analytic cylinder-side radial push, profile-edge movement, negative/symmetric extrusion and topology-aware vertex/edge movement are supported.


### Kernel contract and regression coverage
`solidBuild()` returns the display tessellation plus an analytic/topological record. `finalizeBrep()` rebuilds directed half-edges after every vertex or face operation and rejects open boundaries, non-manifold triangle edges, degenerate triangles and non-positive signed volume. `body.vertexOps`, `body.edits` and `body.faceOps` are feature data, not baked mesh mutations; changing a dimension, source sketch, workplane frame, signed height or transform invalidates and replays the recipe.

The offline suite covers primitive closure/volume, half-edge twins, profile-following extrusions, signed and tilted extrusions, face push/inset/pocket, cylinder-side radius edits, box side/face movement, vertex operation persistence, every box edge fillet, cap/vertical/corner fillet and chamfer combinations, slot arcs, normal winding, transforms and save/load. It also regresses oversized simultaneous top/bottom treatments on a narrow concave 12-edge profile, complete cap triangulation, mixed cap/profile selections, and multi-hole cap bridges. Run `node Verification/smoke.js`; the current baseline is reported by the script (435 checks at the time of this update).
...
A new document starts with an active **Workplane01** on the lattice; `?demo` in the URL loads the sample scene. `?demo` is the only mode that seeds the showcase figures.
...
- **Push commits what the preview shows.** Pushing a face (E / gizmo) first tries the parametric route (height / base / profile-edge / radius edit); the result is compared with the previewed push feature and, if they differ (e.g. the cap has filleted edges, so a taller extrude would swallow the fillet), the previewed push feature is kept instead. Extrusion accepts signed heights, so a negative value grows from the profile in the opposite normal direction; symmetric mode keeps the profile centred.
