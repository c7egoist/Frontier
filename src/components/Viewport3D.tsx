...
export const Viewport3D: React.FC<Viewport3DProps> = ({
  ...
  useEffect(() => {
    ...
    // 5. Generator
    const generator = new BuildingGenerator(config);
    ...
    // 8. Animation & Render Loop with FPS Tracking
    let frameCount = 0;
  ...
  useEffect(() => {
    ...
    // Update wireframe state if enabled
    applyWireframe(root, isWireframe);
  ...
  const setupEnvironment = (scene: THREE.Scene, envMode: string) => {
    // Remove previous non-building lights
    ...
    switch (envMode) {
      case 'cyberpunk_night': {
        ...
        // Deep blue ambient night fill
        const amb = new THREE.AmbientLight('#1d2d44', 0.7);
        scene.add(amb);
  ...
  return (
    <div ref={containerRef} className="relative w-full h-full overflow-hidden bg-[#0a0d14]">
      ...
      {/* Bottom Left Live Stats Badge (AAA Game Engine Inspector) */}
      <div className="absolute bottom-4 left-4 z-10 bg-[#121824]/90 backdrop-blur-md p-3 rounded-xl border border-slate-700/60 shadow-2xl text-xs space-y-1.5 pointer-events-none">
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-slate-400 font-mono text-[11px]">
          ...
          <div>LOD Target: <span className="text-emerald-400">UE5 Nanite</span></div>
        </div>
      </div>
    </div>
  );
