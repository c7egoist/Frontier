...
import { PRESETS, PRESET_GROUPS, TreeParams, cloneParams, SHAPE_NAMES, Shape, scatterFor, trunkBaseRadius, rootReach, isGrass, isSucculent } from './tree/params';
import { GrassParams, Inflorescence, INFLORESCENCE_NAMES, HEAD_BRANCH_NAMES, grassHeight, grassHabit } from './plant/grassParams';
import { SucculentParams, SucculentHabit, HABIT_NAMES, succulentHeight, succulentHabit } from './plant/succulentParams';
...
scroll.append(pages.botany, pages.grass, pages.succulent, pages.roots, pages.mesh, pages.view, pages.report);
const tabMap: [HTMLButtonElement, HTMLElement][] = [
  [tabBotany, pages.botany],
  [tabGrass, pages.grass],
  [tabSucculent, pages.succulent],
...
const succulentTabs = new Set<HTMLButtonElement>([tabSucculent]);
...
function syncTabs(): void {
  ...
  const succulent = isSucculent(params);
  for (const [b] of tabMap) {
    const hide = grass ? treeTabs.has(b) || succulentTabs.has(b) : succulent ? treeTabs.has(b) || grassTabs.has(b) : grassTabs.has(b) || succulentTabs.has(b);
    b.style.display = hide ? 'none' : '';
  }
  const active = tabMap.find(([b]) => b.classList.contains('on'))?.[0];
  if (!active || active.style.display === 'none') showTab(grass ? tabGrass : succulent ? tabSucculent : tabBotany);
  inspectorBadge.textContent = grass ? 'grass · welded' : succulent ? 'succulent · welded' : 'tree · welded';
}
showTab(tabBotany);
...
/** Radius of interest around the base: root reach for trees, the crown for grasses, the base for succulents. */
function baseReach(): number {
  if (isGrass(params) && params.grass) return Math.max(0.2, params.grass.crownRadius * 3);
  if (isSucculent(params) && params.succulent) return Math.max(0.2, params.succulent.baseRadius * 3);
...
/** Level names for the Levels legend, by succulent habit. */
function succulentLevelLegend(): [string, string][] {
  const h = params.succulent?.habit;
  if (h === 'pads')
    return [
      ['Base', '#8a5a3c'],
      ['Pads', '#c98b4b'],
...
function groupIcon(name: string): string {
  ...
  if (g.startsWith('Cacti') || g.startsWith('Succulents')) return GROUP_ICON[g] ?? 'cactus';
...
function updateReport(r: GenerateResponse): void {
  ...
  const grass = isGrass(params);
  const succulent = isSucculent(params);
  const small = grass || succulent;
  const big = el('div', 'big');
  const hSmall = r.summary.height < 1;
  big.append(el('span', 'v', hSmall ? (r.summary.height * 100).toFixed(0) : r.summary.height.toFixed(small ? 2 : 1)), el('span', 'u', hSmall ? 'cm' : 'm'), el('span', 'lab', 'height'));
  ...
  } else if (succulent && r.succulent) {
    const s = r.succulent;
    cell(s.pads ? 'pads' : s.leaves ? 'leaves' : 'stems', fmtInt(s.pads || s.leaves || s.stems));
    cell('spines', fmtCompact(s.spines));
    cell('fruits', fmtInt(s.fruits));
  } else {
  ...
  else if (succulent) drawSpark(spark, r.summary.stemsPerLevel.slice(0, 4), 0, succulentLevelLegend().map(([n]) => n.toLowerCase()));
  ...
  const parts = small ? r.summary.stemsPerLevel.slice(0, 4) : [...r.summary.stemsPerLevel.slice(0, 4), r.summary.rootStems];
  ...
  else if (succulent) footRow.append(kvSpan('cactus', 'organs', fmtInt(r.summary.stems)), kvSpan('layers', 'junctions', fmtInt(r.stats.junctions)), el('span', 'sp'), kvSpan('check', ok ? 'clean' : 'issues', '', ok ? 'ok' : 'warn'));
  ...
  if (grass && r.grass) {
    ...
    const s = r.succulent;
    add('Organs welded', `${fmtInt(s.organs)}  (${s.junctions} windows)`);
    add('Stems · pads · leaves', `${fmtInt(s.stems)} · ${fmtInt(s.pads)} · ${fmtInt(s.leaves)}`);
    add('Spines', fmtInt(s.spines));
    add('Fruits', fmtInt(s.fruits));
    add('Organs dropped', fmtInt(s.dropped), s.dropped ? 'warn' : 'ok');
  } else {
...
const GROUP_ICON: Record<string, string> = {
  ...
  'Cacti · columnar': 'cactus',
  'Cacti · barrels & pears': 'cactus',
  'Succulents · rosettes': 'leaf',
};
const GROUP_ACCENT: Record<string, string> = {
  ...
  'Cacti · columnar': '#4fc9a0',
  'Cacti · barrels & pears': '#e2b95b',
  'Succulents · rosettes': '#7ec8e3',
};
...
function speciesMeta(name: string): string {
  ...
  if (isSucculent(p) && p.succulent) {
    const g = p.succulent;
    const h = succulentHeight(g);
    return `${succulentHabit(g)} · ${h < 1 ? `${Math.round(h * 100)} cm` : `${h.toFixed(1)} m`}`;
  }
  const b = p.botany;
...
function buildSucculentPage(): void {
  const page = pages.succulent;
  page.innerHTML = '';
  if (!isSucculent(params) || !params.succulent) return;
  const G = (): SucculentParams => params.succulent!;
  const g = G();
  ...
  const sHabit = section('Habit', page, { hint: HABIT_NAMES[g.habit] });
  select(
    sHabit,
    'Growth form',
    (Object.keys(HABIT_NAMES) as SucculentHabit[]).map((k) => ({ value: k, label: HABIT_NAMES[k] })),
    () => G().habit,
    (v) => (G().habit = v),
    () => {
      onChange();
  ...
  const sBase = section('Base', page);
  slider(sBase, 'Base radius', () => G().baseRadius, (v) => (G().baseRadius = v), { min: 0.03, max: 0.8, step: 0.005, ...cm }, onChange);
  slider(sBase, 'Base height', () => G().baseHeight, (v) => (G().baseHeight = v), { min: 0, max: 0.4, step: 0.005, ...cm }, onChange);
  slider(sBase, 'Sunk into soil', () => G().baseSink, (v) => (G().baseSink = v), { min: 0, max: 0.15, step: 0.002, ...cm }, onChange);
  slider(sBase, 'Dome profile', () => G().baseProfile, (v) => (G().baseProfile = v), { min: 1.2, max: 6, step: 0.1 }, onChange);

  if (g.habit === 'columnar' || g.habit === 'barrel') {
    const sStem = section('Stems', page, { hint: `${g.stems} stem${g.stems === 1 ? '' : 's'}` });
    slider(sStem, 'Stems', () => G().stems, (v) => (G().stems = Math.round(v)), { min: 1, max: 16, step: 1 }, onChange);
    slider(sStem, 'Height', () => G().stemHeight, (v) => (G().stemHeight = v), { min: 0.05, max: 12, step: 0.01, ...cm }, onChange);
    slider(sStem, 'Height variation', () => G().stemHeightV, (v) => (G().stemHeightV = v), { min: 0, max: 0.6, step: 0.01, unit: '±' }, onChange);
    slider(sStem, 'Radius', () => G().stemRadius, (v) => (G().stemRadius = v), { min: 0.008, max: 0.6, step: 0.002, ...cm }, onChange);
    slider(sStem, 'Radius variation', () => G().stemRadiusV, (v) => (G().stemRadiusV = v), { min: 0, max: 0.5, step: 0.01, unit: '±' }, onChange);
    slider(sStem, 'Taper', () => G().stemTaper, (v) => (G().stemTaper = v), { min: 0, max: 0.9, step: 0.02 }, onChange);
    slider(sStem, 'Lean', () => G().stemLean, (v) => (G().stemLean = v), { min: 0, max: 45, step: 0.5, unit: '°' }, onChange);
    slider(sStem, 'Lean variation', () => G().stemLeanV, (v) => (G().stemLeanV = v), { min: 0, max: 30, step: 0.5, unit: '°' }, onChange);
    slider(sStem, 'Curve', () => G().stemCurve, (v) => (G().stemCurve = v), { min: 0, max: 60, step: 1, unit: '°' }, onChange);
    slider(sStem, 'Ribs', () => G().ribs, (v) => (G().ribs = Math.round(v)), { min: 0, max: 30, step: 1 }, onChange);
    slider(sStem, 'Rib depth', () => G().ribDepth, (v) => (G().ribDepth = v), { min: 0, max: 0.4, step: 0.01, unit: '× r' }, onChange);
    slider(sStem, 'Rib sharpness', () => G().ribSharp, (v) => (G().ribSharp = v), { min: 0, max: 1, step: 0.05 }, onChange);
    if (g.habit === 'barrel') slider(sStem, 'Barrel profile', () => G().barrel, (v) => (G().barrel = v), { min: 0, max: 1, step: 0.05, title: '0 = column · 1 = globe' }, onChange);

    const sArm = section('Arms', page, { hint: g.arms ? `${g.arms} arms` : 'none' });
    slider(sArm, 'Arms', () => G().arms, (v) => (G().arms = Math.round(v)), { min: 0, max: 12, step: 1 }, onChange);
    slider(sArm, 'Attach from', () => G().armAttachMin, (v) => (G().armAttachMin = v), { min: 0.05, max: 0.9, step: 0.01, title: 'Bottom of the attach zone, fraction of the stem' }, onChange);
    slider(sArm, 'Attach to', () => G().armAttachMax, (v) => (G().armAttachMax = v), { min: 0.1, max: 0.98, step: 0.01, title: 'Top of the attach zone, fraction of the stem' }, onChange);
    slider(sArm, 'Length', () => G().armLength, (v) => (G().armLength = v), { min: 0.05, max: 6, step: 0.01, ...cm }, onChange);
    slider(sArm, 'Length variation', () => G().armLengthV, (v) => (G().armLengthV = v), { min: 0, max: 0.8, step: 0.02, unit: '±' }, onChange);
    slider(sArm, 'Radius', () => G().armRadius, (v) => (G().armRadius = v), { min: 0.15, max: 0.9, step: 0.01, unit: '× stem', title: 'Arm radius relative to the stem radius' }, onChange);
    slider(sArm, 'Upturn', () => G().armCurve, (v) => (G().armCurve = v), { min: 0, max: 120, step: 1, unit: '°', title: 'Bend of the arm towards the vertical' }, onChange);
    slider(sArm, 'Arm ribs', () => G().armRibs, (v) => (G().armRibs = Math.round(v)), { min: 0, max: 20, step: 1, title: '0 = inherit from the stem' }, onChange);
    stepper(sArm, 'Branching depth', () => G().branching, (v) => (G().branching = v), { min: 0, max: 3, integer: true, title: '0 = arms are terminal · 2 = cholla joints' }, onChange);
  }

  if (g.habit === 'pads') {
    const sPad = section('Pads', page, { hint: `${g.pads} pads` });
    slider(sPad, 'Pads', () => G().pads, (v) => (G().pads = Math.round(v)), { min: 1, max: 60, step: 1, title: 'Total pads on the plant' }, onChange);
    slider(sPad, 'Length', () => G().padLength, (v) => (G().padLength = v), { min: 0.05, max: 0.8, step: 0.005, ...cm }, onChange);
    slider(sPad, 'Length variation', () => G().padLengthV, (v) => (G().padLengthV = v), { min: 0, max: 0.6, step: 0.01, unit: '±' }, onChange);
    slider(sPad, 'Width', () => G().padWidth, (v) => (G().padWidth = v), { min: 0.02, max: 0.5, step: 0.002, ...cm }, onChange);
    slider(sPad, 'Thickness', () => G().padThickness, (v) => (G().padThickness = v), { min: 0.04, max: 0.5, step: 0.01, unit: '× w' }, onChange);
    slider(sPad, 'Curve', () => G().padCurve, (v) => (G().padCurve = v), { min: 0, max: 45, step: 1, unit: '°' }, onChange);
    slider(sPad, 'Child angle', () => G().padAngle, (v) => (G().padAngle = v), { min: 5, max: 80, step: 1, unit: '°', title: 'Angle between a child pad and its parent face' }, onChange);
    slider(sPad, 'Children per pad', () => G().padBranch, (v) => (G().padBranch = v), { min: 1, max: 5, step: 0.5 }, onChange);
    stepper(sPad, 'Depth', () => G().padDepth, (v) => (G().padDepth = v), { min: 1, max: 5, integer: true, title: 'Maximum recursion depth from the basal pads' }, onChange);
  }

  if (g.habit === 'rosette') {
    const sLeaf = section('Leaves', page, { hint: `${g.leaves} leaves` });
    slider(sLeaf, 'Leaves', () => G().leaves, (v) => (G().leaves = Math.round(v)), { min: 3, max: 80, step: 1 }, onChange);
    slider(sLeaf, 'Length', () => G().leafLength, (v) => (G().leafLength = v), { min: 0.02, max: 2.5, step: 0.005, ...cm }, onChange);
    slider(sLeaf, 'Length variation', () => G().leafLengthV, (v) => (G().leafLengthV = v), { min: 0, max: 0.6, step: 0.01, unit: '±' }, onChange);
    slider(sLeaf, 'Width', () => G().leafWidth, (v) => (G().leafWidth = v), { min: 0.006, max: 0.4, step: 0.002, ...cm }, onChange);
    slider(sLeaf, 'Thickness', () => G().leafThickness, (v) => (G().leafThickness = v), { min: 0.05, max: 0.8, step: 0.01, unit: '× w' }, onChange);
    slider(sLeaf, 'Keel', () => G().leafKeel, (v) => (G().leafKeel = v), { min: 0, max: 1, step: 0.05, title: 'V-fold along the midrib' }, onChange);
    slider(sLeaf, 'Arch', () => G().leafCurve, (v) => (G().leafCurve = v), { min: 0, max: 120, step: 1, unit: '°' }, onChange);
    slider(sLeaf, 'Arch variation', () => G().leafCurveV, (v) => (G().leafCurveV = v), { min: 0, max: 60, step: 1, unit: '°' }, onChange);
    slider(sLeaf, 'Lean', () => G().leafLean, (v) => (G().leafLean = v), { min: 0, max: 90, step: 1, unit: '°', title: 'Tilt from the vertical of the outer leaves' }, onChange);
    slider(sLeaf, 'Twist', () => G().leafTwist, (v) => (G().leafTwist = v), { min: 0, max: 180, step: 1, unit: '°' }, onChange);
    slider(sLeaf, 'Twist variation', () => G().leafTwistV, (v) => (G().leafTwistV = v), { min: 0, max: 90, step: 1, unit: '°' }, onChange);
    slider(sLeaf, 'Teeth per side', () => G().leafTeeth, (v) => (G().leafTeeth = Math.round(v)), { min: 0, max: 40, step: 1 }, onChange);
    slider(sLeaf, 'Tooth size', () => G().leafToothSize, (v) => (G().leafToothSize = v), { min: 0, max: 0.03, step: 0.0005, ...mm }, onChange);
    slider(sLeaf, 'Terminal spine', () => G().terminalSpine, (v) => (G().terminalSpine = v), { min: 0, max: 0.12, step: 0.001, ...mm, title: '0 = none' }, onChange);
  }

  if (g.habit !== 'rosette') {
    const sSp = section('Areoles & spines', page, { hint: g.spinesPerAreole ? `${g.spinesPerAreole} per areole` : 'spineless' });
    slider(sSp, 'Areole spacing', () => G().areoleSpacing, (v) => (G().areoleSpacing = v), { min: 0.008, max: 0.12, step: 0.001, ...mm }, onChange);
    slider(sSp, 'Spines per areole', () => G().spinesPerAreole, (v) => (G().spinesPerAreole = Math.round(v)), { min: 0, max: 5, step: 1 }, onChange);
    slider(sSp, 'Spine length', () => G().spineLength, (v) => (G().spineLength = v), { min: 0.001, max: 0.12, step: 0.001, ...mm }, onChange);
    slider(sSp, 'Length variation', () => G().spineLengthV, (v) => (G().spineLengthV = v), { min: 0, max: 0.8, step: 0.02, unit: '±' }, onChange);
    slider(sSp, 'Spine radius', () => G().spineRadius, (v) => (G().spineRadius = v), { min: 0.0001, max: 0.004, step: 0.00005, format: (v) => (v * 1000).toFixed(2), unit: 'mm' }, onChange);
    slider(sSp, 'Spine angle', () => G().spineAngle, (v) => (G().spineAngle = v), { min: 10, max: 120, step: 1, unit: '°', title: 'Angle between a spine and the parent axis (90 = perpendicular)' }, onChange);
    slider(sSp, 'Spine curve', () => G().spineCurve, (v) => (G().spineCurve = v), { min: 0, max: 120, step: 1, unit: '°', title: 'Bend along the spine (hooked centrals)' }, onChange);
    slider(sSp, 'Central spine', () => G().centralSpine, (v) => (G().centralSpine = v), { min: 0, max: 3, step: 0.1, unit: '×', title: 'Length multiplier of the central spine (0 = all radials)' }, onChange);
    if (g.habit === 'columnar' || g.habit === 'barrel') {
      slider(sSp, 'Spine zone from', () => G().spineZoneMin, (v) => (G().spineZoneMin = v), { min: 0, max: 0.95, step: 0.01, title: 'Bottom of the spiny zone, fraction of the stem' }, onChange);
      slider(sSp, 'Spine zone to', () => G().spineZoneMax, (v) => (G().spineZoneMax = v), { min: 0.05, max: 1, step: 0.01, title: 'Top of the spiny zone, fraction of the stem' }, onChange);
    }
    slider(sSp, 'Areole mound', () => G().areoleMound, (v) => (G().areoleMound = v), { min: 0, max: 1.5, step: 0.05, unit: '×', title: 'Felt mound height as a fraction of the spine length' }, onChange);
  }

  const sFr = section('Fruits', page, { hint: g.fruits ? String(g.fruits) : 'none' });
  slider(sFr, 'Fruits', () => G().fruits, (v) => (G().fruits = Math.round(v)), { min: 0, max: 40, step: 1 }, onChange);
  slider(sFr, 'Fruit radius', () => G().fruitRadius, (v) => (G().fruitRadius = v), { min: 0.005, max: 0.06, step: 0.001, ...mm }, onChange);
  slider(sFr, 'Fruit length', () => G().fruitLength, (v) => (G().fruitLength = v), { min: 0.01, max: 0.15, step: 0.002, ...mm }, onChange);

  const sRes = section('Resolution', page, { collapsed: true });
  if (g.habit === 'columnar' || g.habit === 'barrel') {
    stepper(sRes, 'Stem sides', () => G().stemSides, (v) => (G().stemSides = v), { min: 8, max: 96, step: 2, integer: true, title: 'Vertices around a stem (rounded to a multiple of the rib count)' }, onChange);
    stepper(sRes, 'Stem rings', () => G().stemSegments, (v) => (G().stemSegments = v), { min: 4, max: 60, integer: true }, onChange);
  }
  if (g.habit === 'pads') {
    stepper(sRes, 'Pad sides', () => G().padSides, (v) => (G().padSides = v), { min: 6, max: 16, step: 2, integer: true }, onChange);
    stepper(sRes, 'Pad rings', () => G().padSegments, (v) => (G().padSegments = v), { min: 4, max: 30, integer: true }, onChange);
  }
  if (g.habit === 'rosette') {
    stepper(sRes, 'Leaf sides', () => G().leafSides, (v) => (G().leafSides = v), { min: 6, max: 12, step: 2, integer: true, title: '6 keeled · 8 keeled and wide' }, onChange);
    stepper(sRes, 'Leaf rings', () => G().leafSegments, (v) => (G().leafSegments = v), { min: 4, max: 30, integer: true }, onChange);
  }
  stepper(sRes, 'Spine rings', () => G().spineSegments, (v) => (G().spineSegments = v), { min: 1, max: 6, integer: true }, onChange);
  ...
  page.append(
    Object.assign(el('p', 'note'), {
      textContent:
        'A desert plant is one closed quad surface, like the trees and grasses. A basal dome (or a single basal ring) carries the primary organs; arms, child pads, spines and fruits leave their parent through windows cut into the parent\'s ring grid and are welded to it with collar loops, so the wind deformation runs continuously from the soil to the tip of every spine.',
    }),
  );
}
...
(window as unknown as { frontier: unknown }).frontier = {
  ...
  setParams(partial: {
    ...
    if (partial.succulent && params.succulent) Object.assign(params.succulent, partial.succulent);
