...
/** What kind of plant a parameter set describes. Trees go through the Weber-Penn skeleton, grasses through the grass mesher, succulents through the succulent mesher. */
...
export interface TreeParams {
  ...
  grass?: GrassParams;
  /** Succulent description (kind === 'succulent'). */
...
/** Every species: trees first, then the grasses, then the succulents & cacti. */
export const PRESETS: TreeParams[] = [...TREE_PRESETS, ...GRASS_PRESETS.map((g) => grassPreset(g.name, g.grass)), ...SUCCULENT_PRESETS.map((g) => succulentPreset(g.name, g.succulent))];
