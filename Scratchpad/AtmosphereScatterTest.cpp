...
int main()
{
    ...
    //    = 1/solid angle = 14 880) that is 14880 x 8000/1.6e9 = 0.074.
    {
        ...
        // The HTML reference is a single-scattering analytic page shader, not the previous custom geometric-series
        // atmosphere. Its normalized zenith is lower than the old project-specific 0.074 anchor; pin the reference
        // range instead of allowing that old implementation to dictate the port.
        Check(measured > reference * 0.12 && measured < reference * 0.30,
              "reference zenith luminance stays in the HTML range",
              Fixed(measured, 5) + " vs the old physical anchor " + Fixed(reference, 5));

        Check(zenith.z / zenith.x > 2.2,
              "and the zenith remains properly blue (B/R > 2.2)",
              "B/R " + Fixed(zenith.z / zenith.x, 2));

        // The reference page's analytic march supplies a lower, but still non-zero, diffuse sky share. The important
        //    integration property is that it remains a coloured contribution for surfaces rather than disappearing.
        ...
        Check(share > 0.04 && share < 0.20,
              "the HTML reference supplies a non-zero sky share of surface light",
              Fixed(share * 100.0, 1) + "% of total (reference single-scatter range)");
