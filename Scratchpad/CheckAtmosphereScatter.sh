...
# ── The exact HTML reference terms ───────────────────────────────────────────────────────────────────────────────────────────────
echo
echo "[AtmosphereScatter] the sultanaladin reference march"
CheckConstant "quadratic view sampling (reference s0²/s1² mapping)" 's0 \*= s0; s1 \*= s1' "$Shader"
CheckConstant "Mie extinction is 1.1x scattering"                 'kMieExtinctionScale 1\.1|kMieExtinctionScale \*' "$Shader"
CheckConstant "ozone is in the reference optical-depth ledger"   'betaO \* odR' "$Shader"
CheckConstant "transmittance returns zero through the planet"     'if \(groundHit > 0\.0\) return vec3\(0\.0\)' "$Shader"
CheckConstant "sun ray uses the far shell hit"                    'AtmosphereRaySphereFar' "$Shader"
CheckConstant "planet shadow is checked inside the sun march"    'sunAltitude < 0\.0' "$Shader"
