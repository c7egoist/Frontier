...
int main(int argc, char** argv)
{
    ...
    //---------------------------------------------------------------------------------------------------------
    // 🔴 AIM THE CAMERA AT THE ACTIVE CELESTIAL BODY, not at a fixed compass bearing.
    //
    //    The first version faced a fixed north-east and held the pitch at the horizon. At noon that put the sun
    //    63.7 degrees off-axis, so zeroing the disc changed literally zero bytes. The falsification test was right
    //    to fire: an image that cannot show the disc cannot be evidence. Follow the ephemeris in both azimuth and
    //    elevation; at twilight use the moon when it is up, and at full night use the zenith for stars.
    const vec3 sunWorldDirection = vec3(solution.SunDirection[0], solution.SunDirection[1], solution.SunDirection[2]);
    const vec3 moonWorldDirection = vec3(solution.MoonDirection[0], solution.MoonDirection[1], solution.MoonDirection[2]);
    const vec3 subjectDirection = sunWorldDirection.z > -0.34f ? sunWorldDirection
                                  : moonWorldDirection.z > 0.0f ? moonWorldDirection
                                                               : vec3(0.0f, 0.0f, 1.0f);
    const float subjectAzimuth = std::atan2(subjectDirection.x, subjectDirection.y);
    const float subjectElevation = std::asin(std::clamp(subjectDirection.z, -1.0f, 1.0f));
    const float viewAzimuth = subjectAzimuth + 0.16f;
    const float viewCosElevation = std::cos(subjectElevation);

    CameraBasis camera;
    camera.Origin     = vec3(0.0f, 0.0f, 1.75f);
    camera.Forward    = normalize(vec3(std::sin(viewAzimuth) * viewCosElevation,
                                       std::cos(viewAzimuth) * viewCosElevation,
                                       std::sin(subjectElevation)));
        ...
        for (int s = 0; s < samples; ++s)
        {
            ...
            else
            {
                ...
                colour = colour + CelestialLensFlare(sky, dir, camera.Forward, camera.Right, camera.Up,
                                                     sky.SunTransmittance.xyz(), 1.0f, camera.TanHalfFov);
            }
