...
- Clicking a selected vertex in 3D reveals the dependency-free transform gizmo. Press `G` for move, `S` for scale, or `R` for rotate; only the active operation's handles are rendered and selectable. Move shows red X, green Y, blue Z, translucent plane handles, and a white camera-facing centre ring for screen-space translation. Scale shows RGB cylinders, while rotate shows RGB annular handles. Dragging a move axis changes only the active vertex; Ctrl snaps movement to the current step. Per-vertex depth movement is preserved, and a closed face is left unfilled when that individual move makes it non-planar.
