...
// A6b adaptive-exposure metering. These mirror LuminanceReduce.slang; the gate checks they still agree.
//    A HISTOGRAM rather than a running sum, because a percentile of the frame's own distribution has no
//    absolute constant in it and therefore behaves identically under floodlights and under candlelight. The
//    fixed 1e-2 cd/m² floor it replaced could not: below it the meter went blind, and a dark interior is
//    entirely below it.
...
static constexpr float    kLuminanceLog2High      =  30.0f;   // 1e9 cd/m², above any luminaire in a scene
...
//
//    That distinction is the whole problem. A Cornell frame with a bare bulb in shot is two populations: a
//    room near 1 cd/m² and a bulb at thousands. Walking about changes how much of the frame the bulb
//    covers, the bright mode slid into and out of the average, and the reading swung 2 to 4.7 stops as the
//    camera moved. Exposure is global, so the ROOM pumped along with the bulb — which is the tell, because a
//    room whose brightness depends on where the camera stands is not a lighting problem at all.
//
//    Widening the percentile fixed the pumping and broke the courtyard scene instead: the lit floor is 41 % of
//    that frame and 2.3 stops above the walls, so trimming enough to exclude a bulb in shot also excluded the
//    floor, and the courtyard metered as its own walls.
//
...
//    spread better than the original percentile, and the courtyard metered at 3075 rather than 1909.
//
//    ⚠️ Six is bounded on both sides. Three to eight all measure identically, so it is not a tuned number; at
//    twelve the bulb comes back inside the window and the pumping returns at 2.08 stops.
...
static constexpr uint32_t kCelestialRecordBytes = 368u;
...
// Presentation pacing; the swapchain maps it onto what the surface supports.
...
struct DispatchConfiguration
{
    ...
    // ── Reserve ─────────────────────────────────────────────────────────────────────────────────────────────────
    // ⚠️ The block is exactly 128 bytes, which is Vulkan's GUARANTEED MINIMUM push-constant size. Anything
    //    further has to become a uniform buffer — growing past 128 would work on a card offering 256 and fail
    //    on one offering only the guarantee, i.e. break on someone else's machine.
    float    ColourSaturation;                                     // [-] 1 normally, 0 in near-black adaptive scenes
...
//    vec3 + float pairs pack to 16 bytes each (4 × 16) followed by 8 uints (32) = 96, plus the reserve block
//    (ColourSaturation + 7 spare uints = 32). 128 bytes is the minimum every Vulkan implementation must offer,
//    so this is exactly full: anything further has to become a uniform buffer.
...
public:
    ...
    // Display settings. Each request is applied at the next present: pacing rebuilds the swapchain with the
    ...
    // Ray-tracing capability (plan v2.1 §3.4): probed once the physical device is chosen. The request defaults
    //    to Auto; the resolved tier is what the renderer builds for.
