...
bool SwapchainExchange::BringSwapchain() noexcept
{
    ...
    //    so STORAGE usage (not universally supported) is not requested.
...
// The luminance reduction folds each frame's HDR image into a histogram the exposure integrator reads
//    back; what follows reports the finished slot's mean so the CPU never stalls on the GPU.
...
float SwapchainExchange::QueryAverageLogLuminance() const noexcept
{
    ...
    //    from a bright SUBJECT: lit ground sits about two stops from the luminaire above it and belongs in the
    //    reading, while a bare bulb sits eleven stops above the room and is a light source in shot.
...
void SwapchainExchange::RecordAndPresent(const DispatchConfiguration& Dispatch) noexcept
{
    ...
    // The first touch of the acquired image is the blit (transfer stage).
...
void SwapchainExchange::RecordComputeCommands(uint32_t ImageOrdinal, const DispatchConfiguration& Dispatch) noexcept
{
    ...
    // ⑥ Swapchain image → PRESENT_SRC. The blit wrote it; present reads it. No render pass remains —
    //    the image goes straight from transfer destination to the present engine.
    {
        VkImageMemoryBarrier Barrier{};
        Barrier.sType                           = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
...
void SwapchainExchange::OnKey(GLFWwindow* Window, int Key, int, int Action, int) noexcept
{
    ...
    // Edit keys are queued as a STREAM for the host. They produce no character, so the
...
void SwapchainExchange::OnMouseButton(GLFWwindow* Window, int Button, int Action, int) noexcept
{
    ...
    // The right button drives camera look. Releases always pass so a button cannot stick down.
