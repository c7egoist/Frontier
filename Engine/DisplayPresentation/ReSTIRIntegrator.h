...
//    No sky, sun, moon or stars: ray misses resolve to black and the scene is lit by its own luminaires only,
//    so the ReSTIR estimate can be diagnosed without any environment term moving underneath it.
...
public:
    ...
    // Mutable configuration — set once at startup (CLI flags) or by explicit test code.
    ...
    // A6b ⚠️ Writes BOTH the configuration and the exposure integrator's manual value. Keeping two
    ...
    // A6b. Exposure. Held here because BuildDispatch is what fills the Exposure push constant, so the
    //    configured value and the value the shader receives cannot drift apart.
...
private:
    ...
    ExposureIntegrator Adaptation{};  // A6b: exposure (Manual by default, so the tone map cannot move under ReSTIR)
