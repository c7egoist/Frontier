...
//    behave differently: the atmosphere's aureole grows as the air thickens, while the reference flare's ghosts
//    move when the CAMERA turns even though the air has not changed at all. The shader uses the HTML reference's
//    complete screen-space composite: ghosts, a halo ring, an anamorphic streak and an aperture burst.
...
struct CelestialLensFlare
{
    ...
    // The reference page's halo is a thin chromatic ring, not a broad bloom. Keep the radius in its normalized
    //    screen-space units so the shader can use the exact `abs(length(uv - sunUv * .25) - uHalo)` expression.
    float HaloRadius = 0.55f;     // [-] reference screen-space ring radius
...
inline constexpr CelestialProperty kCelestialProperties[] =
{
    ...
    // ── Lens flare — mapped to the reference page's screen-space lensFlare() controls ────────────────────────
    ...
    FRONTIER_CELESTIAL_REAL("flare.halo.radius", "-", LensFlare.HaloRadius, 0.0f, 2.0f, "reference chromatic ring radius"),
