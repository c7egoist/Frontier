...
int main(int argc, char** argv)
{
    ...
    // The shipping outdoor path follows the HTML reference's elevation-only EV trim. `--sky-exposure` remains
    // accepted as an explicit opt-in for callers that set the flag from an older launch script.
    bool        CelestialExposure = true;
    ...
    while (!Surface.CloseRequested() && (FrameLimit == 0u || PresentedFrames < FrameLimit))
    {
...

        {
            ...
            {
                // Exact reference-page exposure: uEV = exposure.bias and the elevation-only autoEV trim. This
                // value is independent of CameraOrigin, ray direction, cloud cover and ReSTIR state.
                Integrator.Exposure().ObserveCelestialGain(
                    Frontier::ReferenceCelestialExposureGain(Solution.SunElevationDegrees,
                                                              Celestial.Exposure.ExposureBias));
            }
        }
